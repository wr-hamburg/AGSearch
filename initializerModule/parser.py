...
class Parser:
    ...
    def parseInput():
        ...
        parser.add_argument('--searchqueries', required=True, help='Path to the file containing the search query information')
        parser.add_argument('--codequeries', required=True, help='Path to the file containing the code query information')
        parser.add_argument('--dbcollection', nargs='?', default='veryGoodName', help='optional name for the DB collection')
        #If any APIs get added, modify the choices below and the addition to the result
        parser.add_argument('--api', nargs='?', default='rest', choices=['rest', 'graphql'], help='Specify search API (REST or GraphQL)')
        
        args=parser.parse_args()
        
        results = {}
        results.update({'searchqueries' : args.searchqueries})
