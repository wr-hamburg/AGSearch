...
class DBControl:
    ...
    def startContainer(pathToMount):
        ...
        idValue = subprocess.check_output(f'podman run --rm -d -p 27017:27017 -v \'{pathToMount}:/data/db:Z\' mongo', shell=True)
        DBControl._containerId = str(idValue.decode('utf-8')).strip('\n')

        
    @staticmethod
    def stopContainer():
        subprocess.check_output(f'podman container stop {DBControl._containerId}', shell=True)
